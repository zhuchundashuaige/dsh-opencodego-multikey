...
/**
 * Strip a trailing " (Multikey)"-style suffix from a model id.
 * @param model - the model id from the request.
 * @param suffix - the suffix to strip, e.g. " (Multikey)".
 * @returns the base model id, or null when there is no suffix to strip.
 */
export function normalizeMultikeyModel(model, suffix = " (Multikey)") {
	if (typeof model !== "string" || model === "") return null;
	if (typeof suffix !== "string" || suffix === "") return null;
	if (!model.endsWith(suffix)) return null;
	const base = model.slice(0, -suffix.length).trim();
...
/** Compute response headers to forward; keeps content-encoding for transparency. */
export function filteredResponseHeaders(headers, body) {
	const out = {};
	for (const [key, value] of Object.entries(headers ?? {})) {
		const lower = key.toLowerCase();
		if (HOP_BY_HOP.has(lower)) continue;
		// content-length is recomputed, but content-encoding MUST stay: the
		// upstream may return gzip/br bytes and the receiver relies on that
		// header to decompress (stripping it while forwarding the compressed
		// bytes garbles the body into what users saw as a corrupted 404).
...
export async function handleProxy(incoming, outgoing, deps) {
	...
	if (parsedBody !== null && typeof parsedBody === "object" && !Array.isArray(parsedBody) && typeof model === "string" && model !== "") {
		// A " (Multikey)" model is a pool entry point: strip the suffix so the
		// request is served by the real catalog model (and so fallback lookup
		// and usage attribution use the underlying model id).
		const normalized = normalizeMultikeyModel(model, config.multikeySuffix);
		if (normalized !== null) {
			parsedBody = { ...parsedBody, model: normalized };
			model = normalized;
			fromMultikey = true;
			bodyChanged = true;
		}
	...
	if (bodyChanged && parsedBody !== null) {
		body = Buffer.from(JSON.stringify(parsedBody));
		if (fromMultikey) logger?.info?.(`opencodego-multikey: Multikey model entry — routed via key ${keyId} for model ${model}`);
	}
