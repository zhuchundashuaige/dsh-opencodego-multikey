...
export const DEFAULTS = {
	...
	historyDays: 90,
	stateFile: null,
	// -- Multikey supplier injection ----------------------------------------
	// Source provider route whose models are mirrored, and the NEW provider
	// route (with its display name) that gets created pointing at the proxy.
	// The new supplier carries the same models as the source, so it appears as
	// a separate "OpenCode Go Multikey" provider in dsh-usage-stats.
	...
	// Whether to write the new supplier + clean old (Multikey) variants into
	// llm-pi-ai settings on activation. Disable to only run the proxy pool.
	injectProvider: true,
	multikeySuffix: " (Multikey)",
	// Wire protocol the mirrored models speak (inherited from the source route
	// when it declares one; falls back to this).
...
export function normalizeConfig(raw = {}) {
	...
	return {
		...
		sourceRoute: stringOf(source.sourceRoute, DEFAULTS.sourceRoute),
		newProviderRoute: stringOf(source.newProviderRoute, DEFAULTS.newProviderRoute),
		newProviderDisplayName: stringOf(source.newProviderDisplayName, DEFAULTS.newProviderDisplayName),
		...
		fallbackProtocol: stringOf(source.fallbackProtocol, DEFAULTS.fallbackProtocol)
	};
}
...
//#region Multikey supplier injection
/**
 * Return a provider entry with any previously-injected " (Multikey)" variants
 * removed, so the source route stays clean (its baseURL and protocol are
 * preserved as-is).
 * @param provider - a provider object under llm-pi-ai.providers.
 * @param options - { multikeySuffix }.
 * @returns the cleaned provider object (unchanged fields preserved).
 */
export function cleanMultikeyVariants(provider, options = {}) {
	...
	const cleaned = models.filter((entry) => {
		if (entry === null || typeof entry !== "object") return true;
		return !String(entry.id ?? "").endsWith(suffix);
	});
	if (cleaned.length === models.length) return providerObj;
	return { ...providerObj, models: cleaned };
}

/**
 * Build a NEW provider route that mirrors the source route's (cleaned) models
 * and points at the local proxy. The apiKeyEnv is inherited from the source so
 * DSH's config validation passes (the proxy ignores it and uses the Key pool).
 * @param source - the source provider object.
 * @param options - { newProviderRoute, newProviderDisplayName, proxyBaseURL, fallbackProtocol, multikeySuffix }.
 * @returns the new provider object, or null when the source has no usable models.
 */
export function buildMultikeyProvider(source, options = {}) {
	if (source === null || typeof source !== "object") return null;
	const providerObj = Array.isArray(source) ? { models: source } : source;
	const clean = cleanMultikeyVariants(providerObj, options);
	const models = Array.isArray(clean.models) ? clean.models : [];
	const usable = models.filter((entry) => entry !== null && typeof entry === "object" && String(entry.id ?? "") !== "");
	if (usable.length === 0) return null;
	const route = options.proxyBaseURL ?? DEFAULTS.proxyBaseURL;
	const protocol = typeof providerObj.api === "string" ? providerObj.api : options.fallbackProtocol ?? DEFAULTS.fallbackProtocol;
	// Mirror each model with the fields a hand-declared route requires.
	const mirrored = usable.map((entry) => ({
		id: String(entry.id),
		...
		...(entry.reasoningEfforts !== void 0 ? { reasoningEfforts: entry.reasoningEfforts } : {})
	}));
	return {
		apiKeyEnv: typeof providerObj.apiKeyEnv === "string" && providerObj.apiKeyEnv !== "" ? providerObj.apiKeyEnv : "OPENCODE_GO_API_KEY",
		displayName: options.newProviderDisplayName ?? DEFAULTS.newProviderDisplayName,
		api: protocol,
		baseURL: typeof route === "string" && route.trim() !== "" ? route.trim() : DEFAULTS.proxyBaseURL,
		models: mirrored
	};
}

/**
 * Build the full modified providers map: clean the source route's (Multikey)
 * variants and add/replace the new "OpenCode Go Multikey" provider.
 * @param providers - the current llm-pi-ai.providers object.
 * @param options - injection options (see buildMultikeyProvider / cleanMultikeyVariants).
 * @returns { providers, changed } the new providers map plus whether it differs.
 */
export function buildMultikeyProviders(providers, options = {}) {
	const current = providers && typeof providers === "object" ? { ...providers } : {};
	const sourceRoute = options.sourceRoute ?? DEFAULTS.sourceRoute;
	const source = current[sourceRoute];
	if (source === void 0 || source === null) {
		return { providers: current, changed: false, reason: `source provider "${sourceRoute}" not present` };
	}
	const cleanedSource = cleanMultikeyVariants(source, options);
	const next = { ...current, [sourceRoute]: cleanedSource };
	const newProvider = buildMultikeyProvider(source, options);
	if (newProvider !== null) {
		next[options.newProviderRoute ?? DEFAULTS.newProviderRoute] = newProvider;
	}
	const changed = JSON.stringify(next) !== JSON.stringify(current);
	return { providers: next, changed, reason: changed ? "ok" : "unchanged" };
}

/**
 * Idempotently write the new supplier (and clean old Multikey variants) into
 * the harness llm-pi-ai settings via the settings service.
 * @param settings - the `settings` service (ctx.get('settings')).
 * @param options - injection options (see buildMultikeyProviders).
 * @returns { changed, reason } describing what happened.
 */
export async function injectProviderModels(settings, options = {}) {
	...
	const result = buildMultikeyProviders(providers, options);
	if (!result.changed) return { changed: false, reason: "unchanged" };
	const nextNs = { ...(current !== null && typeof current === "object" ? current : {}), providers: result.providers };
	await settings.update(ns, { providers: result.providers });
...
export async function apply(ctx, rawConfig = {}) {
	...
	// --- Multikey supplier injection -----------------------------------------
	// Add a new "OpenCode Go Multikey" provider (mirroring the source route's
	// models and pointing at the proxy) so choosing it in the Model dropdown
	// routes the request through the pool and shows up cleanly in
	// dsh-usage-stats. Also removes any legacy " (Multikey)" variants from the
	// source route. Best-effort: failures only log, never crash the plugin.
	if (config.injectProvider) {
		void injectProviderModels(ctx.get("settings"), {
			sourceRoute: config.sourceRoute,
			newProviderRoute: config.newProviderRoute,
			newProviderDisplayName: config.newProviderDisplayName,
			proxyBaseURL: config.proxyBaseURL,
			multikeySuffix: config.multikeySuffix,
			fallbackProtocol: config.fallbackProtocol
		}).then((result) => {
			ctx.logger?.info?.(`opencodego-multikey: supplier injection ${result.changed ? "applied" : "no-op"} (${result.reason ?? "unchanged"})`);
		}).catch((error) => {
			ctx.logger?.warn?.(`opencodego-multikey: supplier injection failed: ${String(error)}`);
		});
	}
