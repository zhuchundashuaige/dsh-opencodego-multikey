...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".ogk_guide{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:color-mix(in srgb,var(--ogk-accent) 6%,transparent);border-radius:10px;padding:9px 11px;margin:6px 0;font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary);display:flex;flex-direction:column;gap:4px}",
			".ogk_guideTitle{color:var(--dsw-alias-label-primary);font-size:12px;font-weight:600;line-height:18px}",
			".ogk_guide b{color:var(--dsw-alias-label-primary);font-weight:600}",
			".ogk_guide code{font-family:ui-monospace,monospace;font-size:11px;background:var(--dsw-alias-fill-l2);border-radius:4px;padding:0 4px}",
		...
		const S = {
			...
			addForm: "ogk_addForm", addRow: "ogk_addRow", input: "ogk_input", addBtn: "ogk_addBtn",
			guide: "ogk_guide", guideTitle: "ogk_guideTitle", footerNote: "ogk_footerNote"
		};
		//#endregion
		...
		//#region shared gateway data hook
		/**
		 * Shared data/actions for both the sidebar floating panel and the
		 * Settings page entry — keeps the two entry points in sync.
		 */
		function useGateway() {
			...
			const removeKey = (id, label, confirmText) => {
				if (typeof window.confirm === "function" && !window.confirm(`${confirmText} ${label}?`)) return;
				setBusy(true);
			...
			return {
				data, error, busy, keyInput, labelInput, refreshedAt,
				load, act, forceRefresh,
				setKeyInput, setLabelInput,
				addKey, removeKey
			};
		}

		/** Aggregate totals row shared by both entry points. */
		function GatewayAggregate({ aggregate, translate }) {
		...
		/** Key pool + add-form body shared by both entry points. */
		function GatewayKeys({ data, translate, busy, onToggle, onClear, onRemove, onAdd, keyInput, labelInput, setKeyInput, setLabelInput }) {
				...
				children: [
					...
					keys.length === 0 ? react_jsx_runtime.jsx("p", { className: S.quotaEmpty, children: translate("keys.empty") }) : react_jsx_runtime.jsx("div", {
						...
						children: keys.map((key) => react_jsx_runtime.jsx(KeyCard, {
							...
							translate,
							busy,
							onToggle: (enabled) => onToggle(key.id, enabled),
							onClear: () => onClear(key.id),
							...
							react_jsx_runtime.jsxs("div", {
								...
								children: [
									...
									react_jsx_runtime.jsx("button", {
										type: "button", className: S.addBtn, onClick: onAdd, disabled: busy || keyInput.trim() === "",
		...
		/** Settings-page entry point: Key Pool management (usage shown by dsh-usage-stats). */
		function OpenCodeGoMultiKeySettings({ t }) {
			const translate = (key, params) => interpolate(t !== void 0 ? t(key) : key, params);
			const g = useGateway();
			const data = g.data;
			return react_jsx_runtime.jsx("div", {
				className: S.body,
				"data-opencodego-multikey-settings": true,
				children: [
					g.error !== null ? react_jsx_runtime.jsxs("div", {
						className: S.error,
						children: [
							react_jsx_runtime.jsx("span", { children: translate("error", { message: g.error }) }),
							react_jsx_runtime.jsx("button", { type: "button", className: S.retry, onClick: g.load, children: translate("action.retry") })
						]
					}) : null,
					data === null && g.error === null ? react_jsx_runtime.jsx("p", { className: S.note, children: translate("panel.loading") }) : null,
					react_jsx_runtime.jsx("p", { className: S.note, children: translate("settings.desc") }),
					react_jsx_runtime.jsxs("div", {
						className: S.guide,
						children: [
							react_jsx_runtime.jsx("span", { className: S.guideTitle, children: translate("guide.title") }),
							react_jsx_runtime.jsx("span", { children: translate("guide.usage") }),
							react_jsx_runtime.jsx("span", { children: translate("guide.custom") }),
							react_jsx_runtime.jsx("span", { children: translate("guide.note") })
						]
					}),
					react_jsx_runtime.jsx(GatewayKeys, {
						data, translate, busy: g.busy,
						onToggle: (id, enabled) => g.act("keys/toggle", { id, enabled }),
						onClear: (id) => g.act("keys/clear-quarantine", { id }),
						onRemove: (key) => g.removeKey(key.id, key.label, translate("action.deleteConfirm")),
						onAdd: g.addKey,
						keyInput: g.keyInput, labelInput: g.labelInput,
						setKeyInput: g.setKeyInput, setLabelInput: g.setLabelInput
					}),
					g.refreshedAt !== null && react_jsx_runtime.jsx("p", {
						className: S.footerNote,
						children: `${translate("panel.updatedAt")} ${new Date(g.refreshedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`
					})
				]
			});
		}
		...
		function OpenCodeGoMultiKeyPanel({ t }) {
			...
			const g = useGateway();
			const { data, error, busy, keyInput, labelInput, refreshedAt } = g;
						...
						children: [
										...
										children: [
											react_jsx_runtime.jsx(primitives.Tooltip, {
												...
												children: react_jsx_runtime.jsx("button", {
													...
													onClick: g.forceRefresh, disabled: busy,
							...
							react_jsx_runtime.jsx("div", {
								...
								children: [
									...
									react_jsx_runtime.jsx("p", { className: S.note, children: translate("panel.usageHint") }),
									react_jsx_runtime.jsx("section", {
										className: S.section,
										children: react_jsx_runtime.jsx("h3", { className: S.sectionTitle, children: translate("keys.title") })
									}),
									keys.length === 0 ? react_jsx_runtime.jsx("p", { className: S.quotaEmpty, children: translate("keys.empty") }) : react_jsx_runtime.jsx("div", {
										...
										children: keys.map((key) => react_jsx_runtime.jsx(KeyCard, {
											...
											translate,
											busy,
											onToggle: (enabled) => g.act("keys/toggle", { id: key.id, enabled }),
											onClear: () => g.act("keys/clear-quarantine", { id: key.id }),
											onRemove: () => g.removeKey(key.id, key.label, translate("action.deleteConfirm"))
										}))
									}),
									react_jsx_runtime.jsx("section", {
										className: S.section,
									...
									react_jsx_runtime.jsxs("div", {
										...
										children: [
											react_jsx_runtime.jsx("input", {
												type: "password", className: S.input, value: keyInput,
												placeholder: translate("add.keyPlaceholder"),
												onChange: (event) => g.setKeyInput(event.target.value),
												onKeyDown: (event) => { if (event.key === "Enter") g.addKey(); }
											}),
											react_jsx_runtime.jsxs("div", {
												className: S.addRow,
												children: [
													react_jsx_runtime.jsx("input", {
														type: "text", className: S.input, value: labelInput,
														placeholder: translate("add.labelPlaceholder"),
														onChange: (event) => g.setLabelInput(event.target.value),
														onKeyDown: (event) => { if (event.key === "Enter") g.addKey(); }
													}),
													react_jsx_runtime.jsx("button", {
														type: "button", className: S.addBtn, onClick: g.addKey, disabled: busy || keyInput.trim() === "",
														children: translate("add.submit")
													})
												]
											})
					...
					react_jsx_runtime.jsx("div", {
						...
						children: react_jsx_runtime.jsxs("button", {
							...
							children: [
								...
								keys.length > 0 && react_jsx_runtime.jsx("span", { className: S.badgeCount, children: String(keys.length) })
							]
						})
					})
				]
		...
		const zh = {
			...
			"panel.badge": "Go 多Key",
			"panel.updatedAt": "更新于",
			"panel.loading": "正在加载网关状态…",
			"panel.usageHint": "使用用量请见侧边栏「用量/余额」(dsh-usage-stats)——本面板仅管理 API Key 池。",
			"settings.nav": "OpenCodeGo 多Key",
			"settings.desc": "管理 OpenCodeGo API Key 池，按剩余额度自动调度；用量由 dsh-usage-stats 展示。",
			"guide.title": "如何查看汇总用量",
			"guide.usage": "1. 在模型下拉框选择带 (Multikey) 后缀的模型（如 minimax-m3 (Multikey)），调用会自动对 Key 池负载均衡。",
			"guide.custom": "2. 若你使用自定义供应商的模型：在 DSH 的 Models 页面为该供应商配置模型（或在本插件 providers 列表添加自定义模型），这些模型会出现在 dsh-usage-stats 中按 model 展示。",
			"guide.note": "3. 打开侧边栏「用量/余额」(dsh-usage-stats) 即可按 (Multikey)/自定义模型查看汇总 Usage。",
			"error": "加载失败：{message}",
			"action.refresh": "刷新用量",
			"action.retry": "重试",
			"action.close": "关闭",
		...
		const en = {
			...
			"panel.loading": "Loading gateway…",
			"panel.usageHint": "Usage is shown in the sidebar \"Usage/Balance\" panel (dsh-usage-stats) — this panel only manages the API-key pool.",
			"settings.nav": "OpenCodeGo Multi-Key",
			"settings.desc": "Manage the OpenCodeGo API-key pool; requests automatically use the key with the most remaining quota.",
			"guide.title": "How to view aggregate usage",
			"guide.usage": "1. In the Model dropdown pick a model with the (Multikey) suffix (e.g. minimax-m3 (Multikey)); calls automatically load-balance the key pool.",
			"guide.custom": "2. Using custom-supplier models? Configure them for the provider on DSH's Models page (or add custom models to this plugin's providers list); they then appear in dsh-usage-stats per model.",
			"guide.note": "3. Open the sidebar \"Usage/Balance\" (dsh-usage-stats) to view aggregate usage by (Multikey)/custom model.",
		...
		function apply(ctx) {
			...
			// Settings-page entry (the only entry point): bilingual nav label,
			// re-read on locale change. No sidebar badge — see Settings → OpenCodeGo 多Key.
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				inject: () => ({ t })
			}, (props) => react.createElement(OpenCodeGoMultiKeySettings, {
				t: props.t,
				close: props.close
			})));
		}
		...
		exports.OpenCodeGoMultiKeySettings = OpenCodeGoMultiKeySettings;
		exports.GatewayKeys = GatewayKeys;
		exports.GatewayAggregate = GatewayAggregate;
