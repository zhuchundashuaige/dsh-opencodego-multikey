...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"guide.usage": "1. 插件启动后会自动新增供应商「OpenCode Go Multikey」（模型与 OpenCode Go 一致）。在模型下拉框选择该供应商的任何模型，调用即自动对 Key 池负载均衡。",
			"guide.custom": "2. 若你使用自定义供应商的模型：在 DSH 的 Models 页面为该供应商配置模型，这些模型会出现在 dsh-usage-stats 中按 model 展示。",
			"guide.note": "3. 打开 dsh-usage-stats「用量/余额」，即可看到「OpenCode Go Multikey」各模型的汇总 Usage。",
			"error": "加载失败：{message}",
			"action.refresh": "刷新用量",
			"action.retry": "重试",
			"action.close": "关闭",
		...
		const en = {
			...
			"guide.usage": "1. On load the plugin adds a provider \"OpenCode Go Multikey\" (models identical to OpenCode Go). Pick any of its models in the dropdown; calls automatically load-balance the key pool.",
			"guide.custom": "2. Using custom-supplier models? Configure them on DSH's Models page; they then appear in dsh-usage-stats per model.",
			"guide.note": "3. Open dsh-usage-stats \"Usage/Balance\" to view aggregate usage for each \"OpenCode Go Multikey\" model.",
