...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"settings.desc": "管理 OpenCodeGo API Key 池，按剩余额度自动调度，并查看每个 Key 的用量与汇总。",
			"error": "加载失败：{message}",
			"action.refresh": "刷新用量",
			"action.retry": "重试",
			"action.close": "关闭",
		...
		const en = {
			...
			"settings.desc": "Manage the OpenCodeGo API-key pool; requests automatically use the key with the most remaining quota. Track per-key and aggregate usage.",
		...
		function apply(ctx) {
			...
			// Settings page: bilingual nav label, re-read on locale change.
			...
			ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register({
				...
				id: "opencodego-multikey",
				locale: NS,
				// order 2: rendered ABOVE the usage-stats entry (order 10) so this
				// plugin's badge stays visible instead of being crowded off the
				// narrow sidebar row.
				order: 2
